...
public class VncViewer extends java.applet.Applet
  ...
  // Does server support XVP extensions to RFB protocol
  boolean xvpExtensions;
  // Should we show XVP control buttons
  boolean xvpShutdown;
  boolean xvpReboot;
  boolean xvpReset;
  ...
  public void run() {
	...
	vncFrame.setTitle("XVP Viewer - " + rfb.desktopName);
	vncFrame.pack();
	...
	if (inSeparateFrame) {
	  vncFrame.setTitle("XVP Viewer - "+rfb.desktopName+" [disconnected]");
	}
  ...
  void connectAndAuthenticate() throws Exception
  {
    ...
    case RfbProto.AuthVNC:
      ...
      if (passwordParam.length > 0) {
        ...
        rfb.authenticateVNC(pw.getBytes());
      }
      break;
    case RfbProto.AuthXVP:
      showConnectionStatus("Performing XVP authentication");
      byte[] pass;
      if (passwordParam.length > 0) {
	pass = passwordParam;
      } else {
	...
	pass = pw.getBytes();
      ...
      // user and vm could be non-ASCII, but password must be ASCII
      rfb.authenticateXVP(user.getBytes("UTF-8"), vm.getBytes("UTF-8"), pass);
      break;
    default:
  ...
  void setEncodings(boolean autoSelectOnly) {
    ...
    encodings[nEncodings++] = RfbProto.EncodingXVP;
  ...
  void readParameters() {
    ...
    // user and vm are XVP-specific
    user = readParameter("USER", false);
    if (user == null)
      user = "";
    vm = readParameter("VM", false);
    if (vm == null) {
	vm = "";
    } else if (vm.startsWith("UTF-8:")) {
      // Decode "UTF-8:xx:xx:xx:xx....xx" xvpweb encoding option
      String vmvalues[] = vm.split(":");
      byte vmbytes[] = new byte[vmvalues.length - 1];

      for (int i = 1; i < vmvalues.length; i++) {
	vmbytes[i-1] = (byte)Integer.parseInt(vmvalues[i], 16);
      }
      try {
	String newvm = new String(vmbytes, "UTF-8");
	vm = newvm;
    ...
    readOnly = false;
    str = readParameter("Read Only", false);
    ...
    // "xvp..." set to "No" disables XVP buttons in button panel.
    xvpShutdown = true;
    str = readParameter("xvpshutdown", false);
    if (str != null && str.equalsIgnoreCase("No"))
      xvpShutdown = false;
    xvpReboot = true;
    str = readParameter("xvpreboot", false);
    if (str != null && str.equalsIgnoreCase("No"))
      xvpReboot = false;
    xvpReset = true;
    str = readParameter("xvpreset", false);
    if (str != null && str.equalsIgnoreCase("No"))
      xvpReset = false;
  ...
  private void readPasswordParameters() {
    ...
    if ((param = readParameter("XVPPASSWORD", false)) != null) {
      //
      // XVP extension, XVPPASSWORD is hexascii, fixed width
      // width, may contain zeros, to be used as is.
      //
      byte[] pw = {0, 0, 0, 0, 0, 0, 0, 0};
      for (int i = 0; i < 8; i++) {
        String hex = param.substring(i*2, i*2+2);
    ...
    if ((param = readParameter("ENCPASSWORD", false)) != null) {
      ...
      param = new String(pw);
    } else if ((param = readParameter("PASSWORD", false)) == null) {
      passwordParam = new byte[0];
      return;
    }

    if (param.length() > 8)
      param = param.substring(0, 8);	// Truncate to 8 chars

    // Truncate password on the first zero byte.
    int firstZero = param.indexOf(0);
    if (firstZero != -1)
      param = param.substring(0, firstZero);

    passwordParam = param.getBytes();
  }
  ...
  public void updateXVP(int code) throws Exception {
    System.out.println("Server sent XVP code " + code);
    if (code == rfb.XVPCodeInit) {
      xvpExtensions = true;

      if (showControls && !readOnly) {
	if (xvpShutdown)
	  buttonPanel.enableXVPShutdown(true);
	if (xvpReboot)
	  buttonPanel.enableXVPReboot(true);
	if (xvpReset)
	  buttonPanel.enableXVPReset(true);
      }

    } else if (code == rfb.XVPCodeFail) {
      XvpConfirmDialog.confirmed(this, "fail");
    } else {
      throw new Exception("Unknown XVP code " + code);
    }
  }
}
