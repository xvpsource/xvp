...
class RfbProto {
  ...
  final static int
    ...
    EncodingXVP            = 0xFFFFFECB; // officially allocated for XVP
  ...
  void authenticateVNC(byte[] pw) throws Exception {
  ...
  void authenticateXVP(byte user[], byte target[], byte[] pw) throws Exception {
  ...
  String readServerCutText() throws IOException {
    ...
    readFully(text);
    // Colin Dean added "UTF-8" parameter, but XenServer consoles
    // (at least Linux ones), send back gibberish for non US ASCII,
    // so this doesn't actually help
  ...
  //
  // Re-written by Colin Dean to use UTF-8, which then works, at least
  // for XenServer Linux consoles, for characters even outside Latin-1. 
  //

  void writeClientCutText(String text) throws IOException {
    byte[] t = text.getBytes("UTF-8");
    byte[] b = new byte[8 + t.length];

    b[0] = (byte) ClientCutText;
    b[4] = (byte) ((t.length >> 24) & 0xff);
    b[5] = (byte) ((t.length >> 16) & 0xff);
    b[6] = (byte) ((t.length >> 8) & 0xff);
    b[7] = (byte) (t.length & 0xff);

    System.arraycopy(t, 0, b, 8, t.length);
    os.write(b);
  }
  ...
  void writeKeyEvent(KeyEvent evt) throws IOException {
    ...
    // From 2010-08-27 TightVNC patch ID 3054359 by Albert Fluegel
    ...
    // Generalised by Colin Dean for all non alphanumeric keys, so other
    // keyboards with Alt-Gr, e.g. French and Swedish, work.
    if ((modifiers & CTRL_MASK) != 0 && (modifiers & ALT_MASK) != 0
	&& ((key >= 0x21 && key <= 0x2f) ||
	    (key >= 0x3a && key <= 0x40) ||
	    (key >= 0x5b && key <= 0x60) ||
	    (key >= 0x7b && key <= 0x7e))) {
      String shiftsyms = "~!@#$%^&*()_+|{}:\"<>?";
      modifiers = (shiftsyms.indexOf(key) == -1) ? 0 : SHIFT_MASK;

    // With Ctrl+Shift the backend expects uppercase chars in the key event
    } else if ((modifiers & CTRL_MASK) != 0 && (modifiers & SHIFT_MASK) != 0) {
      key = Character.toUpperCase(key);
    } else if ((modifiers & CTRL_MASK) == 0 && (modifiers & ALT_MASK) == 0
	// Colin Dean: fudge shift state for symbols in the US ASCII range
	// as if entered on a US PC keyboard, even if not.  The aim is to
	// fix a number of symbols that otherwise don't work on e.g. a
	// German keyboard when used with XenServer.
	//
	// See list discussion March 2012 with Robert Riedl et al.
	//
	&& ((key >= 0x21 && key <= 0x2f) ||
	    (key >= 0x3a && key <= 0x40) ||
	    (key >= 0x5b && key <= 0x60) ||
