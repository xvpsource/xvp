...
//
// Extensions to support mouse wheel and XVP server
...
class VncCanvas extends Canvas
  ...
  public VncCanvas(VncViewer v, int maxWidth_, int maxHeight_)
    ...
    inputEnabled = false;
    if (!viewer.readOnly && !viewer.options.viewOnly)
      enableInput(true);
  ...
  public void processNormalProtocol() throws Exception {
	...
	break;
      ...
      case RfbProto.ServerXVPCode:
        int code = rfb.readServerXVPCode();
	viewer.updateXVP(code);
	break;
  ...
  public void processLocalMouseEvent(MouseEvent evt, boolean moved) {
    ...
    // Patch 2012-05-07 from Valters Švābe <vsvabe@gmail.com>:
    //
    // On some setups (like Firefox and Chrome on Ubuntu with
    // OpenJDK/IcedTea-Web) the console applet stops accepting keyboard
    // input after losing focus. The only solution is to reload the page
    // or in some cases to restart the browser.
    //
    // I have narrowed the cause down to an unnecessary
    // MouseEvent.consume call which prevents the applet from regaining
    // keyboard focus when clicked. Removing this call solves the
    // problem.
    //
    //evt.consume();
  }
